...
function setupTabSwitching() {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            // If the target pane doesn't exist on this page, let the browser navigate normally
            ...
            switch (tabId) {
                ...
                case 'cutting':
                    ...
                    break;
                case 'production':
                    viewTitle.innerText = "Ishlab Chiqarish & Tikuv Liniyalari";
                    actionButtonHTML = `<span class="badge" style="background:var(--border-color); color:white; padding:8px 12px; font-size:0.8rem;"><i class="fa-solid fa-shirt"></i> Tikuv Bo'limi Nazorati</span>`;
                    break;
                case 'paint':
...
function renderProduction() {
    ...
    let activeA = false, activeB = false, activeC = false;

    state.orders.forEach(order => {
        const percent = Math.round((order.tayyorlandi / order.miqdori) * 100);

        if (order.status === 'Tikilmoqda') {
            if (order.liniya === 'Liniya A') activeA = true;
            if (order.liniya === 'Liniya B') activeB = true;
            if (order.liniya === 'Liniya C') activeC = true;
        }

        let statusBadgeHTML = '';
        if (order.status === 'Rejalashtirildi') {
            statusBadgeHTML = `<span class="badge-row badge-status-prospect">Rejalashtirilgan</span>`;
        } else if (order.status === 'Bichildi') {
            statusBadgeHTML = `<span class="badge-row badge-status-qualified">Bichilgan</span>`;
        } else if (order.status === 'Tikilmoqda') {
            statusBadgeHTML = `<span class="badge-row badge-status-negotiation">Tikilmoqda</span>`;
        } else {
            statusBadgeHTML = `<span class="badge-row badge-status-won">Tayyor</span>`;
        }

        let actionHTML = '';
        if (order.status === 'Rejalashtirildi') {
            actionHTML = `<button class="btn btn-primary btn-block btn-xs" onclick="transitionOrderStatus('${order.id}', 'Bichildi')"><i class="fa-solid fa-scissors"></i> Bichuvga o'tkazish</button>`;
        } else if (order.status === 'Bichildi') {
            actionHTML = `<button class="btn btn-primary btn-block btn-xs" onclick="transitionOrderStatus('${order.id}', 'Tikilmoqda')"><i class="fa-solid fa-play"></i> Tikishni boshlash</button>`;
        } else if (order.status === 'Tikilmoqda') {
            actionHTML = `<span style="font-size:0.75rem; color:var(--color-negotiation);"><i class="fa-solid fa-hourglass-half"></i> Tikuv bo'limida ishlanmoqda</span>`;
        } else {
            actionHTML = `<span style="font-size:0.75rem; color:var(--color-won);"><i class="fa-solid fa-circle-check"></i> Partiya yakunlandi</span>`;
        }

        const card = document.createElement('div');
        card.className = 'prod-order-card';
        card.innerHTML = `
            <div class="prod-card-header">
                <div class="prod-card-title">
                    <h4>${order.mahsulot_nomi}</h4>
                    <span>Kod: <code>${order.id}</code> | Liniya: ${order.liniya}</span>
                </div>
                ${statusBadgeHTML}
            </div>
            <div class="prod-card-body">
                <div class="prod-meta-row">
                    <span class="prod-meta-lbl">Mahsulot turi:</span>
                    <span class="prod-meta-val">${order.turi}</span>
                </div>
                <div class="prod-meta-row">
                    <span class="prod-meta-lbl">Boshlangan sana:</span>
                    <span class="prod-meta-val">${order.boshlangan_sana}</span>
                </div>
                <div class="prod-meta-row">
                    <span class="prod-meta-lbl">Topshirish sanasi:</span>
                    <span class="prod-meta-val">${order.topshirish_sana}</span>
                </div>
                
                <div class="prod-progress-wrapper">
                    <div class="progress-header">
                        <span>Tikildi: <strong>${order.tayyorlandi} / ${order.miqdori}</strong></span>
                        <span>${percent}%</span>
                    </div>
    ...
    document.getElementById('prod-stat-lineA').innerText = activeA ? "Faol" : "Bo'sh";
    document.getElementById('prod-stat-lineA').parentElement.querySelector('.dot-indicator').style.backgroundColor = activeA ? "var(--color-won)" : "var(--color-text-muted)";
    
    document.getElementById('prod-stat-lineB').innerText = activeB ? "Faol" : "Bo'sh";
    document.getElementById('prod-stat-lineB').parentElement.querySelector('.dot-indicator').style.backgroundColor = activeB ? "var(--color-won)" : "var(--color-text-muted)";
    
    document.getElementById('prod-stat-lineC').innerText = activeC ? "Faol" : "Bo'sh";
    document.getElementById('prod-stat-lineC').parentElement.querySelector('.dot-indicator').style.backgroundColor = activeC ? "var(--color-won)" : "var(--color-text-muted)";
}
